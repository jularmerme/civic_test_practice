...
export class UI {
  ...
  renderQuestion(options: {
    ...
    document.querySelectorAll('.answer-option').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        const answer = (e.currentTarget as HTMLElement).getAttribute('data-answer');
        if (answer) {
  ...
  renderSettings(options: {
    ...
    document.getElementById('dark-mode-toggle')?.addEventListener('change', (e) => {
      options.onToggleDarkMode((e.target as HTMLInputElement).checked);
    });
    document.getElementById('65-20-toggle')?.addEventListener('change', (e) => {
      options.onToggle65_20((e.target as HTMLInputElement).checked);
    });
    document.getElementById('immediate-feedback-toggle')?.addEventListener('change', (e) => {
      options.onToggleImmediateFeeback((e.target as HTMLInputElement).checked);
    });
